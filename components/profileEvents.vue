<template>
  <div>
    ...
    <section class="profile__events-section">
      <h2 class="visually-hidden">Events</h2>
      <ul v-if="storedEvents.length" class="profile-info__events-list">
        <li
...
export default {
  ...
  beforeMount() {
    ...
    this.$store.dispatch('events/fetchPageEvents', '_page').then(() => {
  ...
  methods: {
    getNextPage() {
      this.handleFilterChange(
        ...
        `page=${this.events.currentPage + 1}`
      )
    },
    cancelTime() {
      this.isTime = false
    ...
    handleFilterChange(e, page = 'page=1') {
      ...
      let addressSrting =
        this.currentFilterType !== 'false'
          ? `_page?${page}&type[]=` + this.currentFilterType + '&'
          : `_page?${page}&`
      ...
      this.$store.dispatch('events/fetchPageEvents', addressSrting).then(() => {
        this.events.data.forEach((event) => {
          this.storedEvents.push(event)
        })
      })
